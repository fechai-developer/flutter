...
begin
  ...
  -- Perfis (o trigger já criou as linhas; aqui ajustamos nome + telefone + PIX)
  update profiles set name = 'Ana Prado',  phone = '11911112222', pix_key = 'ana@email.com' where id = v_ana;
  update profiles set name = 'Bruno Lima', phone = '11933334444', pix_key = '11933334444'    where id = v_bruno;
  ...
  insert into group_members (group_id, profile_id, name, phone, status)
    values (g_id, v_voce, 'Você', null, 'accepted') returning id into gm_voce;
  insert into group_members (group_id, profile_id, name, phone, status)
    values (g_id, v_ana, 'Ana Prado', '11911112222', 'accepted') returning id into gm_ana;
  -- Bruno entra como PENDENTE de confirmação (para testar o banner de convite, #1)
  insert into group_members (group_id, profile_id, name, phone, status)
    values (g_id, v_bruno, 'Bruno Lima', '11933334444', 'pending') returning id into gm_bruno;
