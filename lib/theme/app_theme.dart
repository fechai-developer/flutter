...
class AppColors {
  ...
  static const Color textoSuave = Color(0xFF5B726C); // texto secundário, mais leve que 70% opacity

  // Fundo — branco levemente mint, bem claro e arejado
  static const Color nevoaClara = Color(0xFFF5FBF9);
  static const Color superficieCartao = Color(0xFFFFFFFF); // cards sobre o fundo

  // Bordas / neutros — hairline suave (moderno e leve)
  static const Color areiaNeutra = Color(0xFFEAF1EE);
  static const Color areiaNeutraForte = Color(0xFFDCE6E2); // quando precisa de um traço mais visível
...
class AppTheme {
  ...
  /// Sombra suave e difusa — dá profundidade "flutuante" sem pesar. Duas
  /// camadas (uma rente, uma ampla) para um look moderno e leve.
  static List<BoxShadow> softShadow({Color? color}) {
    final c = color ?? AppColors.verdeAguaProfundo;
    return [
      BoxShadow(color: c.withValues(alpha: 0.04), blurRadius: 4, offset: const Offset(0, 2)),
      BoxShadow(color: c.withValues(alpha: 0.05), blurRadius: 24, offset: const Offset(0, 12)),
    ];
  }

  /// Sombra colorida sutil para o card de saldo (gradiente verde-água).
  static List<BoxShadow> get balanceShadow => [
        BoxShadow(
          color: AppColors.verdeAguaProfundo.withValues(alpha: 0.28),
          blurRadius: 24,
  ...
  static TextTheme _textTheme(Brightness brightness) {
    ...
    final Color onSurface = isLight ? AppColors.tintaProfunda : AppColors.nevoaClaraDarkText;
    final Color muted = isLight ? AppColors.textoSuave : onSurface.withValues(alpha: 0.7);
    ...
    return TextTheme(
      ...
      bodySmall: _body(TextStyle(fontSize: 12, color: muted)),
